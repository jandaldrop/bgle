...
class BGLEIntegrator:
    ...
    def __init__(self,
        ...
        self.noise_generator = ColoredNoiseGenerator(
    ...
    def f_rk(self, x, v, rmi, fr, next_w, last_w, last_v, last_rmi):
        nv = v
        na = (-next_w * rmi - last_w * last_rmi - 0.5 * next_w * self.kernel[0]
              * v * self.dt - 0.5 * last_w * self.kernel[0] * last_v * self.dt
              - self.dU(x) + fr) / self.m
        return nv, na

    def rk_step(self, x, v, rmi, fr, last_v, last_rmi):
        k1x, k1v = self.f_rk(x, v, rmi, fr, 0.0, 1.0, last_v, last_rmi)
        k2x, k2v = self.f_rk(x + k1x * self.dt / 2, v + k1v * self.dt / 2, rmi,
                             fr, 0.5, 0.5, last_v, last_rmi)
        k3x, k3v = self.f_rk(x + k2x * self.dt / 2, v + k2v * self.dt / 2, rmi,
                             fr, 0.5, 0.5, last_v, last_rmi)
        k4x, k4v = self.f_rk(x + k3x * self.dt, v + k3v * self.dt, rmi, fr,
                             1.0, 0.0, last_v, last_rmi)
        return x + self.dt * (
